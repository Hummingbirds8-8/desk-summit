...
const GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbyhJ0bokhuG350LJ18xBFUIm5BXeXd_G-l-m87i9PahVpQQ-ftaJAQBgVfwq7Vqk1xtAg/exec";
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const EXERCISES = {
    ...
    plank: {
      name: "フロント・プランク (Plank)",
      desc: "【自宅・体幹】うつ伏せから肘とつま先で体を支え、一本の板のように一直線にキープ！",
      duration: 60,
      climb: 600,
      exp: 30,
      calories: 20,
      breathPattern: [
        { phase: '肘をつき、体を一直線にしてキープ！', duration: 15, type: 'hold' },
        { phase: 'お腹とお尻に力を入れ、呼吸は止めずに！', duration: 15, type: 'hold' },
        { phase: '腰が落ちないように体幹を維持！耐えて！', duration: 15, type: 'hold' },
        { phase: 'ラスト15秒！限界を超えてキープ！', duration: 15, type: 'hold' }
      ]
    },
    widesquat: {
      name: "ワイド・スクワット (Wide Squats)",
      desc: "足を肩幅より広く開き、つま先を外側に向けて腰を落とします。内もも（内転筋）をキュッと引き締め！",
      duration: 40,
      climb: 400,
      exp: 20,
      calories: 18,
      breathPattern: [
        { phase: '足を肩幅より広く開き、つま先を外側にする', duration: 5, type: 'inhale' },
        { phase: 'ゆっくり息を吐きながら、腰を深く落とす', duration: 8, type: 'exhale' },
        { phase: '太ももが平行になる位置でキープ！内もも意識！', duration: 7, type: 'hold' },
        { phase: '息を吸いながらゆっくりと立ち上がる', duration: 5, type: 'inhale' },
        { phase: 'もう一度吐きながら腰を落とす！', duration: 8, type: 'exhale' },
        { phase: '内ももを引き締めながらキープ！', duration: 7, type: 'hold' }
      ]
    },
  ...
  function completeExerciseSession() {
    ...
    // Send log data to GAS Web App (asynchronous background request)
    if (typeof GAS_WEBAPP_URL !== 'undefined' && GAS_WEBAPP_URL && GAS_WEBAPP_URL !== "YOUR_GAS_URL_HERE") {
      const now = new Date();
      ...
      const payload = {
        ...
        durationOrReps: ex.duration + "秒",
        earnedElevation: metersClimbed + "m"
      };
      ...
      .then(response => {
        if (!response.ok) {
          console.warn("GAS log response not ok");
        }
      })
      .catch(err => {
        console.error("Failed to send GAS fitness log:", err);
      });
    }
  }
